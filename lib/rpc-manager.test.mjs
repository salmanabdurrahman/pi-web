...
test("exports getRpcSessionCount for health endpoint", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  assert.match(source, /export function getRpcSessionCount/);
  assert.match(source, /getRpcSessionCount/);
});

test("exports getRunningRpcSessionIds for session list", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  // Should already be exported and used by the sessions API
