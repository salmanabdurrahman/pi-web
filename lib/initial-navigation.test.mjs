...
test("restores session when cwd is absent", async () => {
  ...
  assert.deepEqual(getInitialNavigation(new URLSearchParams({ session: "saved-session" })), {
    requestedCwd: null,
...
test("preserves a URL-encoded Windows path", async () => {
  ...
  assert.deepEqual(getInitialNavigation(new URLSearchParams("cwd=C%3A%5CProjects%5Cpi-web")), {
