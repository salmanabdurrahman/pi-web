...
test("redactValue recurses arrays", async () => {
  const { redactValue } = await loadSubject();
  const input = {
    providers: [
  ...
  assert.deepEqual(result.providers, [
    { name: "openai", apiKey: "<redacted>" },
    { headers: [{ Authorization: "<redacted>" }, { cookie: "<redacted>" }] },
  ]);
});

test("looksSecret matches OAuth/session/header fields", async () => {
  const { looksSecret } = await loadSubject();
  assert.equal(looksSecret("accessToken"), true);
  assert.equal(looksSecret("refresh_token"), true);
  assert.equal(looksSecret("idToken"), true);
  assert.equal(looksSecret("cookie"), true);
  assert.equal(looksSecret("session"), true);
  assert.equal(looksSecret("bearer"), true);
  assert.equal(looksSecret("authorization"), true);
});
