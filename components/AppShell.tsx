...
import { PiRuntimeConfig } from "./PiRuntimeConfig";
...
export function AppShell() {
  ...
  const [piRuntimeOpen, setPiRuntimeOpen] = useState(false);
      ...
      >
        ...
        <div style={{ display: "flex", flex: 1, overflow: "hidden", position: "relative" }}>
          ...
          >
            ...
            <div className="flex h-[36px] shrink-0 items-center gap-4 border-b border-[var(--border)] bg-[var(--bg-panel)] px-2 text-[12px] font-medium text-[var(--text-muted)]">
              {(["files", "context", "tools"] as const).map((mode) => (
                <button
                  key={mode}
                  onClick={() => setRightPanelMode(mode)}
                  className={`transition-colors hover:text-[var(--text)] ${rightPanelMode === mode ? "-mb-[2px] border-b-2 border-[var(--accent)] pb-[2px] text-[var(--text)]" : ""}`}
                >
